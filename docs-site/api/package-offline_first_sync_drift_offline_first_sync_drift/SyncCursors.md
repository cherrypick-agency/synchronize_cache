---
title: "SyncCursors"
description: "API documentation for SyncCursors class from offline_first_sync_drift"
category: "Classes"
library: "offline_first_sync_drift"
sourceUrl: "https://github.com/cherrypick-agency/synchronize_cache/blob/main/packages/offline_first_sync_drift/lib/src/tables/cursors.dart#L7"
outline: [2, 3]
editLink: false
prev: false
next: false
...
<div class="member-signature"><pre><code><span class="kw">class</span> <span class="fn">SyncCursors</span> <span class="kw">extends</span> <span class="type">Table</span></code></pre></div>

**Annotations:** `@UseRowClass(SyncCursorData)`

Cursor table for stable pull pagination.
Stores the last sync position for each kind.

:::info Inheritance
Object → HasResultSet → Table → **SyncCursors**
:::

## Constructors {#section-constructors}

### SyncCursors() {#ctor-synccursors}

<div class="member-signature"><pre><code><span class="fn">SyncCursors</span>()</code></pre></div>
...
you override [primaryKey](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#prop-primarykey). You can turn this behavior off if you want to.
...
that affects [operator ==](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#operator-equals) comparisons.

All objects have hash codes.
The default hash code implemented by [Object](https://api.flutter.dev/flutter/dart-core/Object-class.html)
represents only the identity of the object,
the same way as the default [operator ==](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#operator-equals) implementation only considers objects
equal if they are identical (see [identityHashCode](https://api.flutter.dev/flutter/dart-core/identityHashCode.html)).

If [operator ==](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#operator-equals) is overridden to use the object state instead,
the hash code must also be changed to represent that state,
otherwise the object cannot be used in hash based data structures
like the default [Set](https://api.flutter.dev/flutter/dart-core/Set-class.html) and [Map](https://api.flutter.dev/flutter/dart-core/Map-class.html) implementations.

Hash codes must be the same for objects that are equal to each other
according to [operator ==](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#operator-equals).
...
If a subclass overrides [hashCode](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#prop-hashcode), it should override the
[operator ==](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#operator-equals) operator as well to maintain consistency.
...
### lastId <Badge type="tip" text="no setter" /> {#prop-lastid}

<div class="member-signature"><pre><code><span class="type">TextColumn</span>&lt;<span class="type">String</span>&gt; <span class="kw">get</span> <span class="fn">lastId</span></code></pre></div>
...
When two rows have the same value in *any*  set specified in [uniqueKeys](/api/package-offline_first_sync_drift_offline_first_sync_drift/SyncCursors#prop-uniquekeys),
